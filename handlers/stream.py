...
async def execute_processing(client: Client, user_id: int, message: Message):
    ...
    # index bookkeeping below — everything else is sent as a REPLY to this message
    # so the extracted audio / screenshot / sample visually stay attached to it
    # instead of showing up as unrelated standalone messages in the chat.
    ...
    for idx, (file_to_send, media_type) in enumerate(outputs_to_upload, start=1):
        ...
        # Reply target: once the main renamed video is sent, every subsequent
        # extra (audio/screenshot/sample) replies to it so Telegram threads
        # them together instead of leaving them floating as separate messages.
        reply_to_id = main_video_msg.id if main_video_msg else None

        if media_type == "video" and main_video_msg is None:
            main_video_msg = await client.send_video(
            ...
            # e.g. the sample clip — a second video, threaded under the main upload
            ...
            await client.send_audio(
                chat_id=message.chat.id,
                audio=file_to_send,
                caption=f"<b>🎵 Audio extracted from:</b> <code>{new_name}</code>",
                reply_to_message_id=reply_to_id
            )
            ...
            await client.send_photo(
                chat_id=message.chat.id,
                photo=file_to_send,
                caption=f"<b>📸 Screenshot from:</b> <code>{new_name}</code>",
                reply_to_message_id=reply_to_id
            )
