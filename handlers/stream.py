...
from ffmpeg_tools import get_media_streams, remove_streams, extract_audio, extract_subtitle, take_screenshot, create_sample, extract_stream
from utils import progress_bar, humanbytes
...
async def select_stream_rm(client: Client, callback_query: CallbackQuery):
    ...
    await callback_query.answer(f"Updated selection: {sel}")
...
async def execute_processing(client: Client, user_id: int, message: Message):
    ...
    # 1. Download File if not already downloaded
    ...
    # 2. Execute requested FFmpeg actions
    ...
    # 3. Handle secondary outputs (Audio/Screenshots/Samples)
    ...
    # 4. Upload Files back to Telegram
